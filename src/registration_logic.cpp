...
std::size_t metadata_size_bytes(const std::unordered_map<std::string, std::string>& metadata) {
    std::size_t total = 0;
...
ValidationResult RegistrationValidator::validate(const RegistrationInput& input) const {
    ...
    if (metadata_size_bytes(input.metadata) > 4096) {
        return {false, "validation_error", "metadata не должен превышать 4KB"};
    }
...
ValidationResult validate_registration(const RegistrationInput& input) {
    return RegistrationValidator().validate(input);
}
